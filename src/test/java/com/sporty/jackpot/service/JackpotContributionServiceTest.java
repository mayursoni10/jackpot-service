...
class JackpotContributionServiceTest {
    ...
    void processBet_FixedContribution_CalculatesCorrectly() {
        ...
        // Setup only the fixed strategy for this test
        ContributionStrategy fixedStrategy = (jackpot, betAmount) ->
                betAmount.multiply(jackpot.getContributionPercentage())
                        .divide(new BigDecimal("100"), 2, RoundingMode.HALF_UP);
        when(strategyFactory.getStrategy(ContributionType.FIXED)).thenReturn(fixedStrategy);
    ...
    void processBet_VariableContribution_DecreasesWithPoolSize() {
        ...
        // Setup only the variable strategy for this test
        ContributionStrategy variableStrategy = (jackpot, betAmount) -> {
            ...
            currentPercentage = currentPercentage.max(BigDecimal.ONE); // Minimum 1%

            return betAmount.multiply(currentPercentage)
                    .divide(new BigDecimal("100"), 2, RoundingMode.HALF_UP);
        };
        when(strategyFactory.getStrategy(ContributionType.VARIABLE)).thenReturn(variableStrategy);
